...
namespace Units\Auth\Admin\Services;
...
{
    private $_verify_key = '_verify_admin';
    ...
    public function actSendOTP(string $mobile): self
    {
        ...
        session()->put(session()->id().'state', 'verify');
        session()->put(session()->id().'admin_mobile_phone', $mobile);
        $app=app();
    ...
    public function actValidate(string $username, string $password, string $mobile): self
    {
        ...
        if (UserService::STATUS_ACTIVE != $user->status) {
    ...
    {
        $response = session()->get(session()->id().'admin_mobile_phone');
        if ($response) {
            return $response;
        } else {
            return null;
    ...
    public function actSetLoggedInUser($mobile): self
    {
        ...
        Filament::getPanel('admin')->auth()->login($user);
        Filament::getPanel('admin')->auth()->setUser($user);
        $user = Filament::getPanel('admin')->auth()->user();
        $check = Filament::getPanel('admin')->auth()->check();
