 ...
 * Phone         +989353466620
 * Date          4/28/25, 8:29 AM
 */

namespace Units\Auth\Manage\Filament\Components;
...
use Units\Auth\Manage\Filament\Pages\AuthLayoutComponent;
...
class LoginComponent extends AuthLayoutComponent
{
     ...
     * @var view-string
     */
    protected static string $view = 'manage_auth::filament.pages.auth.login';
...
//        }
            ...
            $this->alert_error('خطای غیر منتظره ای پیش آمد');
    ...
    protected function getForms(): array
    {
        ...
        if (env('ENABLE_DEFAULT_LOGIN_DETAILS',false)){
            $defaults = resolve(DevOnboardingCommand::class)->model_data['manage_user'][0];
        }
        return [
            'form' => $this->form(
                $this->makeForm()
        ...
        return TextInput::make('password')
            ...
            ->hint(
                filament()->hasPasswordReset() ? new HtmlString(
                    Blade::render(
                        '<x-filament::link :href="filament()->getRequestPasswordResetUrl()" tabindex="3"> {{ __(\'filament-panels::pages/auth/login.actions.request_password_reset.label\') }}</x-filament::link>'
                    )
                ) : null
            )
            ->password()
