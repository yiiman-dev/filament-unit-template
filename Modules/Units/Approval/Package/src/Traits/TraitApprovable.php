...
use EightyNine\Approvals\Models\ProcessApproval;
use Exception;
...
//            throw new RuntimeException('فقط کسی که این اطلاعات را بار اول ثبت نموده است میتواند تغییرات جدید اعمال کند!');
//        }
        $user = $user ?? Auth::user();
        try {
            DB::beginTransaction();
    ...
    public function approve($comment = null, Authenticatable|null $user = null): ProcessApproval|bool|RedirectResponse // TODO remove the redirectResponse
    {
        ...
        try {
            ...
            $approval = ProcessApproval::query()->updateOrCreate([
                ...
                'comment' => $comment,
                'user_id' => $user_id,
                'approver_name' => $user?->first_name.' '.$user->last_name ?? 'Unknown'
            ]);
            if ($approval) {
